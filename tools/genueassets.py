...
--pak adds a pak's assets from each .uasset's own export table: a mod pak (mint's mods_P.pak) carries no registry.
Its assets replace the registry's at the same path, as the pak does in game.
"""
import collections
import keyword
import os
...
def folder_rows(folder):
    """An extracted pak's assets as registry rows: FSD/Content/A/B.uasset is /Game/A/B, its asset export's class."""
    rows = []
    for here, _dirs, files in os.walk(folder):
        rel = os.path.relpath(here, folder).replace(os.sep, "/").split("/")
        if "Content" not in rel[:2]:
            continue                                # ponytail: /Game only, a plugin's content is left out
        for f in files:
            if not f.endswith(".uasset"):
                continue
            _names, imports, exports, _deps = dumpedl.read_tables(open(os.path.join(here, f), "rb").read())
            for e in exports:
                if e["is_asset"] and e["class"] < 0:
                    rows.append({"asset_class": imports[-e["class"] - 1][1], "asset_name": e["name"],
                                 "package_name": "/".join(["/Game"] + rel[rel.index("Content") + 1:] + [f[:-7]])})
    return rows


def pak_rows(pak):
    if os.path.isdir(pak):
        return folder_rows(pak)
    with tempfile.TemporaryDirectory() as tmp:
        if subprocess.run([UNREALPAK, os.path.abspath(pak), "-Extract", tmp, "-Filter=*.uasset"],
                          capture_output=True).returncode != 0:
            sys.exit("UnrealPak could not extract %s (UNREALPAK=%s)" % (pak, UNREALPAK))
        return folder_rows(tmp)
...
def main():
    args, paks = sys.argv[1:], []
    while "--pak" in args:
        i = args.index("--pak")
        paks.append(args[i + 1])
        del args[i:i + 2]
    if len(args) != 3:
        sys.exit(__doc__.strip().splitlines()[0])
    registry, ueapi, out = args
    _version, _names, rows, *_ = dumpar.read(registry)
    by_path = {(r["package_name"], r["asset_name"]): r for r in rows}
    for pak in paks:
        by_path.update(((r["package_name"], r["asset_name"]), r) for r in pak_rows(pak))
    ...
    for (cpp, header), assets in sorted(by_class.items()):
        ...
        text = ("#pragma once\n/*\nEvery %s asset in the game, %d of them, generated by AssetGen/tools/genueassets.py from"
                " the game's\nAssetRegistry.bin%s. Do not edit. Each is named by its content path:\n"
                "    &UeAssets::%s\npoints at %s.\nUeAssets::%s::All is every one of them, as soft pointers.\n*/\n"
                "#include \"%s\"\n#include \"%s\"\n\nnamespace UeAssets::%s {\nUE_ASSET_ALL(::%s);\n}\n%s\n"
                % (bare, len(assets), "".join(" and " + os.path.basename(p) for p in paks), example[0], example[1], bare,
                   rel(os.path.join(ueapi, "UeMeta.h")), rel(header),
                   bare, cpp, "\n".join(lines)))
