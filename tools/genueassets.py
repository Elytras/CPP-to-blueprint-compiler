...
so a mod writes `&UeAssets::UMaterialInstanceConstant::Game::...::M_Biome_AzureWeald_Rock01`. The registry is the
...
def main():
    ...
    for (cpp, header), assets in sorted(by_class.items()):
        ...
        text = ("#pragma once\n/*\nEvery %s asset in the game, %d of them, generated by AssetGen/tools/genueassets.py from"
                ...
                "    &UeAssets::%s\npoints at %s.\n*/\n#include \"%s\"\n#include \"%s\"\n\n%s\n"
                % (bare, len(assets), example[0], example[1], rel(os.path.join(ueapi, "UeMeta.h")), rel(header),
                   "\n".join(lines)))
