...
n = 0
for m in ({}, {'a': 1}, {'a': 1, 'b': 2}, {'a': 0, 'b': 0, 'c': 0}, {'a': -50, 'b': 6, 'c': 2**31 - 1}):
    for fn, oracle in (('ReadScoresThrough', read_through), ('StoreScoresThrough', store_through), ('NestScores', nest_scores)):
        mine, theirs = range_self(Scores=m), range_self(Scores=m)
        got = run(asset('RangeTest'), fn, self_vars=mine)[0]
        want = oracle(theirs)
        theirs['Scores'] = {k: i32(v) for k, v in theirs['Scores'].items()}
        assert (got, mine) == (want, theirs), '%s(%s) = %r %r, want %r %r' % (fn, m, got, mine, want, theirs)
        n += 1
    theirs = dict(Scores=dict(m))
    vm = VM(asset('RangeTest'), Scores=dict(m))
    got, want = vm.call('SumScoresViaMethod'), via_method(theirs)
    assert (got, vm.self.vars['Scores']) == (want, theirs['Scores']), ('SumScoresViaMethod', m, got, vm.self.vars, want)
    n += 1
print('ok  RangeTest: a reference TMap loop whose body also uses the map sees Value as the element  (%d cases)' % n)


def weigh_spots(s):
    return sum(v['X'] * len(s) + v['Y'] for v in s.values())
...
for m in ({}, {'a': {'X': 1, 'Y': 2}}, {'a': {'X': -1, 'Y': 0}, 'b': {'X': 5, 'Y': 7}}):
    for fn, oracle in (('WeighSpots', weigh_spots), ('NudgeSpots', nudge_spots)):
        theirs = {k: dict(v) for k, v in m.items()}
        vm = VM(asset('RangeTest'), Spots={k: dict(v) for k, v in m.items()})
        got, want = vm.call(fn), oracle(theirs)
        assert (got, vm.self.vars['Spots']) == (want, theirs), (fn, m, got, vm.self.vars, want, theirs)
        n += 1
for m in ({}, {'a': 1}, {'a': 5, 'b': 7}, {'a': -50, 'b': 6, 'c': 0}):
    near, theirs = Obj('RangeTest_C', Scores=dict(m)), dict(m)
    vm = VM(asset('RangeTest'), Near=near, Scores={'z': 9})
    got, want = vm.call('BumpThroughNear'), bump_through(theirs)
    assert (got, near.vars['Scores'], vm.self.vars['Scores']) == (want, theirs, {'z': 9}), ('BumpThroughNear', m, got, near.vars, want, theirs)
    n += 1
print('ok  RangeTest: a read-only body keeps the copy; a method on another object reaches its map  (%d cases)' % n)
for m in ({}, {'a': {'X': 1, 'Y': 2}}, {'a': {'X': -1, 'Y': 0}, 'b': {'X': 5, 'Y': 7}}):
